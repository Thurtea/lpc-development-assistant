...
#define SAVE_INTERMUD "/save/imud.o"
#define SERVICES_D "/daemon/services"
...
static void Setup() {
    ...
    eventWrite( ({ "startup-req-3", 5, mud_name(), 0, Nameservers[0][0], 0,
                   ...
                   PORT_OOB, PORT_UDP, mudlib() + "" , 
                   mudlib() + "" , driver(), "LP Mud",
...
static void eventRead(mixed *packet) {
    ...
    if( Banned[packet[2]] ) {
        ...
        eventWrite(({ "error", 5, mud_name(), 0, packet[2],
                          packet[3], "unk-user", 
                          "Your mud is not allowed to send to this mud.",
                          packet }));
    ...
    switch(packet[0]) {
        ...
        case "channel-e":
          ...
          break;
        case "channel-m":
          //tell_object(find_player("cratylus"),"hey\n");

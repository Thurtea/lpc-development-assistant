...
mixed query_aquatic(object player)
{
   return -1;
}   
mixed query_swim_stamina(object player)
{
   return (player->query_con()*10)+player->query_property(SWIM_PROP);
}
mixed query_underwater_breathing(object player)
{
    return (player->query_con()*10)+player->query_property(UBREATHE_PROP);    
}
...
void awater(object player)
{
    if(player->query_property("gasping"))
    {
        ...
        player->remove_static_property("gasping");
    }
}        
void gasping(object player)
{
    ...
    if(!player->query_property("gasping"))
    {
    player->add_static_property("gasping",1);
...
void sinking(object player)
{
    ...
    amt = player->query_property("swimming");    
    if(amt)
    {
    if(stamina == -1) // aquatic 
        player->add_static_property("swimming",4);
    else player->add_static_property("swimming",1);
    player->add_timed_property("swimming",2,stamina);  
    }
    else // adjust for encumb.
    {
     dur = query_time_remaining("swimming");
   dur = adjust_stamina(player,dur);
        player->add_timed_property("swimming",amt,dur);   
    }
    switch(player->query_property("swimming"))
    {    
    case 4: // aquatic, surface anyway
    break;
   default:
    environment(player)->DO_SINK(player,environment(player));
    break;
    case 2:
   tell_object(player,"%^RED%^You struggle to stay afloat.%^RESET%^\n");
    break;
    }               
}
void drowning(object player)
{
    ...
    {
    if(stamina == -1) // aquatic 
        player->add_static_property("drowning",4);
    else player->add_static_property("drowning",1);
...
{        
    ...
    if("/obj/handlers/weather"->query_moon_string(ENV(player)) ==
      "full moon")
    {
        player->add_property("old_race",player->query_race());
        player->set_race("/std/races/werewolf");
    }
    }  
}

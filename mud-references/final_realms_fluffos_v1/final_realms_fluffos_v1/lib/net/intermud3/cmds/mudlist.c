...
mixed cmd(string str) {
    ...
    list += ({ sprintf("%:-15s %:-6s %:-15s %:-18s %s %s",
        replace_string(mud,"%^","%%^^"), identify(info[8]), identify(info[7]), identify(info[5]), identify(info[1]), identify(info[2])) });
    ...
    if(this_player())
        this_player()->more_string(implode(list, "\n")+"\n", "MudList");

/*** Poison Vial ***/
/*** Moderate hit Poison type. ***/
/*** by Wonderflug **/

inherit "/std/item";
#define MYPOISON "/std/poisons/shadows/moderate_hit_sh.c"
#define XLEVEL 8
#define MAXUSES 3
...
void setup()
{
  ...
  set_read_mess("\nDemon Ichor.\n\n"
    "Very hard to come by, this disgusting slime is found only on "
    "the most hideous demons from Hell's depths.  The merest touch "
    "is enough to cause incredible pain, the more so if in direct "
 ...
 */
  if ( sizeof(this_player()->query_attacker_list()) )
  {
    ...
    tell_room(this_player(), this_player()->query_cap_name()+
      " fumbles a Vial of Blue Liquid and spills some on "+

...
int set_god_here(string which) { 
  ...
  if(GOD_HAND->is_deity(which)) {
    ...
    if(altar && GOD_HAND) altar->set_long(GOD_HAND->altar_long(which));
    if(altar) altar->move(this_object());
    if(GOD_HAND && GOD_HAND->query_high_priest(which)) {
      bust = clone_object(OBJS+"bust");
      if(bust) bust->subject(GOD_HAND->query_high_priest(which)[0],
                    GOD_HAND->query_high_priest(which)[3],mydeity);
      if(bust) bust->move(this_object());
    }
  }
}

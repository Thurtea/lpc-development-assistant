...
int i;
 for(i=0;i<sizeof(npcs);i++) {
  if(!npcs[i]) {
  npcs[i]=clone_object(DNPC+"dminotaur");
...
add_exit("north",NEWBIE+"dlev3_11","path");
add_exit("south",NEWBIE+"dlev3_19","path");
add_exit("east",NEWBIE+"dlev3_15","path");
add_exit("west",NEWBIE+"dlev3_13","path");
}

/*  Text alterations and additions by Grimbrand                         */
...
int i;
 for(i=0;i<sizeof(npcs);i++) {
  if(!npcs[i]) {
  npcs[i]=clone_object(DNPC+"ddrow");
...
void setup() {
  ...
  npcs = allocate(1);
   ...
   add_exit("north",NEWBIE+"dlev5_04","path");
   add_exit("south",NEWBIE+"dlev5_00","door");
   add_exit("east",NEWBIE+"dlev5_02","path");
   add_exit("west",NEWBIE+"dlev5_03","path");
}

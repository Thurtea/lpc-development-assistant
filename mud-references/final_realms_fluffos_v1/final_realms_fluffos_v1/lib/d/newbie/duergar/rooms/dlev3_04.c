...
int i;
 for(i=0;i<sizeof(npcs);i++) {
  if(!npcs[i]) {
  npcs[i]=clone_object(DNPC+"dliz-man");
...
set_long("\n   Realm of the Duergar:  Surface Proving Grounds.\n\n"
     ...
     "the caverns of your homeland .  There isn't a "
...
add_exit("north", NEWBIE+"dlev3_03", "path");
add_exit("south", NEWBIE+"dlev3_05", "path");
add_exit("east",NEWBIE+"dlev3_01","path");
add_exit("west", NEWBIE+"dlev3_09", "path");
}
void dest_me() {
int i;
for(i=0;i<sizeof(npcs);i++)

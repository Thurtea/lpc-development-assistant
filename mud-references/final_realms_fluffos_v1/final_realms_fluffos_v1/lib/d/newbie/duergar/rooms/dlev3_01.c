...
int i;
 for(i=0;i<sizeof(npcs);i++) {
  if(!npcs[i]) {
  npcs[i]=clone_object(DNPC+"dspider");
...
add_exit("south", NEWBIE+"dlev3_12", "path");
add_exit("west",NEWBIE+"dlev3_04","path");
add_exit("northeast",NEWBIE+"dlev3_02","path");
add_exit("northwest",NEWBIE+"dlev3_03","path");
add_exit("southeast",NEWBIE+"dlev3_00","door");
add_exit("southwest",NEWBIE+"dlev3_05","path");
}

...
int read_me(string str) {
    ...
    this_player()->remove_property("GNOME_PARCHMENT");
    this_player()->remove_property("GNOME_ROSE");
    this_player()->add_property("GNOME_ROPE",1);
    write_file("/d/mm/logs/GNOME_QUEST",this_player()->query_cap_name()+
      " Has completed the rope-skills quest in the Gnome "
      "area on: "+ctime(time())+"\n");
    this_object()->dest_me();
    return 1;
    }

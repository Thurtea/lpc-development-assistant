...
void setup() {
   ...
   add_item(({"wall", "walls"}), "These walls are made from a grey-colored "
          "stone which you have never seen before.\n");

   add_clone(CHAR+"captives", random(4));
   add_smell("room", "It smells old.\n");
   add_taste("air", "The air tastes Musty and old.\n");

   add_exit("north",MAZE+"maze68","corridor");
   add_exit("south",MAZE+"maze90","corridor");
   add_exit("west",MAZE+"maze78","corridor");

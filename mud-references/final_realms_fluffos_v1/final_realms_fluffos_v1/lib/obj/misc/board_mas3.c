...
void init() {
  ...
  add_action("more_list_boards","mboards");
}
...
// This code by Aragorn@NANVAENT (because FR has too many boards :-)
// Modify at will but keep this comment.

int more_list_boards() {
   ...
   my_boards = (string *)BOARD_HAND->list_of_boards();

   news_rc = (mapping)this_player()->query_property(NEWS_RC);
   if (!news_rc) news_rc = ([ ]);

   for(ok1=0;ok1<sizeof(my_boards);ok1++) {
      stuff = (mixed *)BOARD_HAND->get_subjects(my_boards[ok1]);
      s = (sizeof(stuff)>1)?"s":"";

      for (ok2=0;ok2<sizeof(stuff);ok2++)
         if (news_rc[my_boards[ok1]] < stuff[ok2][B_TIME])
            break;

      switch (sizeof(stuff)) {
         case 0:  write(sprintf("%-15s [ Empty ]\n",my_boards[ok1])); break;
         default:
           write(sprintf("%-15s [ "+sizeof(stuff)+
           ((sizeof(stuff)-ok2)?(" note"+s+", "+
            (sizeof(stuff)-ok2)+" unread ]"):" note"+s+" ]")+"\n",
            my_boards[ok1]));
      }
   }
   return 1;

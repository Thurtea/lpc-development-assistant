...
#define HOME "/obj/handlers/weather_room.c"
#define MAX_DIST 100
#define YEAR 168
#define DAY 60
#define SPEED 60
...
string season_string(object env);
string daynight_string(object env);
...
int query_day(object env) {
  ...
  if (!env) {
    env = find_object("/d/ss/daggerford/ladyluck");
    if (!env)
      call_other("/d/ss/daggerford/ladyluck", "??");
    env = find_object("/d/ss/daggerford/ladyluck");
  }
...
string weather_string(object env) {
  return temperature_string(env)+" "+season_string(env)+
    daynight_string(env)+" with "+cloud_string(env)+
    rain_string(env);
}
...
/* String for seasons.  Santino 01/97 */

string season_string(object env) {
  int my_season;
  my_season = query_season();
...
/* Is it day or night? (for general weather string) Santino, 01/97 */

string daynight_string(object env) {
  int daynight;
  daynight = query_day(env);
  if(daynight > 0)
    daynight = 1;
  if(daynight < 0)
    daynight = 1;
...
string temperature_string(object env) {
  ...
  if (inten<-10)
    inten = -10;
  return ({ "It's a hellishly hot", /* -10 */
    "It's an unnaturally hot", /* -9 */
            "Damn hot", /* -8 */
            "Very hot", /* -7 */
            "Hot", /* -6 */
            "Hot", /* -5 */
 ...
 *
 */
void save_weather() {  save_object(FILE_NAME,1); }
